...
predicted = np.sign(predicted)  # Преобразуем прогноз в -1 и 1
accuracy = accuracy_score(train_labels, predicted)
print(f"Точность композиции на обучающей выборке: {accuracy}")
...
plt.title(f"Точность композиции: {accuracy:.2f}")
plt.show()
